...
const ProgressChart = () => {
  ...
  return (
    <div className="space-y-6 px-1 md:px-2 max-w-full overflow-hidden">
      <motion.div 
      ...
      >
        ...
        <ResponsiveContainer width="100%" height="85%" minWidth={250}>
          <BarChart
            data={barData}
            margin={{ top: 10, right: 20, left: -15, bottom: 5 }}
          >
            <XAxis dataKey="name" stroke="#AAAAAA" tick={{fontSize: 12}} />
            <YAxis stroke="#AAAAAA" domain={[0, 100]} tickFormatter={(tick) => `${tick}%`} tick={{fontSize: 12}} width={40} />
      ...
      >
        ...
        <ResponsiveContainer width="100%" height="70%" minWidth={200}>
          <PieChart>
            <Pie
              data={pieData}
              cx="50%"
